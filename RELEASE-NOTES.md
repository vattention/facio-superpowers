...
### facio-user-journey：增加使用规模与逐工程产出统计

- 增加统计跨度、工程数、素材数、素材总时长，以及 Chat / Remotion 的业务次数与 Langfuse generation 次数。
- 增加按 `project_id` 展示的工程证据、素材数量、素材总时长、成片时长和导出结果。
- 增加 `AssetAnalysis.*` 素材去重、完整 Remotion 流程和跨数据源工程归属规则。
- 使用“已确认 / 可观测 / 高置信推断 / 无法归属”区分证据等级，避免将时间窗口推断写成直接事实。
- 保持旧版报告 JSON 兼容；未提供新字段时，对应 HTML 区块自动隐藏。

---

## v2.9.5 (2026-07-17)

### facio-user-journey：增加用户成本归因报告

- 根据部署环境选择对应的 Langfuse project，汇总目标用户的模型调用成本。
- 增加 Remotion、素材分析与 Chat 三类成本占比，并展示全部 `trace_name` 的成本分布。
- 增加成本数据可用性与覆盖范围说明，避免在凭证缺失或查询失败时给出误导性结论。
- 强化报告脱敏与嵌套内容渲染，并补充成本报告自动化测试。
